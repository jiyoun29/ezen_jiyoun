...
public class Day4_2 {
	...
	while(true) {
		...
		if(선택 == 1) {
			...
			//콜라 구매수가 0이 아닐 때에 콜라에 콜라구매수를 넣고 구매수에 따른 가격도 넣는다.
			...
			//구매 했을 때만 장바구니가 나오도록 if로 갈림길 생성 (콜라구매수 !=0 <-같지않다.)
			...
			if( 결정 == 1 ) { //결제 진행
				...
				// 결정이 1일 경우 금액이 총결제액보다 적으면 결제 불가로 나타내고 다시 달라고 하기 위해 while을 추가.
				// 금액이 총결제액보다 같거나 작으면 구매 성공 하고 구매수를 초기화한다. 그 아래 break를 걸어 루프를 탈출하고 while을 종료한다.
